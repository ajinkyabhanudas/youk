...
| `/surface-options` | Decision Surface | Ambiguous/open-ended/judgment-call requests with a real unstated fork — surfaces 2-4 costed options + a mandatory recommendation instead of picking silently. Closes the north-star gap where challenge/adr only engage after a direction exists. | ACTIVE |
...
| 2026-06-27 | Rewrote /humanize voice profile: hard rules for no em dashes, no rhetorical buildup | Extracted from the developer's actual writing patterns |
...
| 2026-09-03 | Added /surface-options (decision surface — fires on ambiguous/open-ended requests, surfaces 2-4 costed options + mandatory recommendation before a direction is picked silently). Registered in `_CAPABILITY_SKILLS` (health.py, session.py) so invocations count toward skill_invocation_rate. Roster: 28 → 29. | North-star self-assessment: goal 1 (developer completeness) had 5 healthy capability skills with measured challenge/learn cohort lift; goal 2 (articulate nuance, nudge deeper insight with options) had no dedicated skill or signal — challenge/adr only engage after a direction already exists. Stress-tested before applying: initial draft NEEDS REVISION on 2 HIGH findings (no interruption budget, undefined dev-loop interaction); revised and retested to SURVIVES. |
