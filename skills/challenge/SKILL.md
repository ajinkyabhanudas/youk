...
- **Naming/framing assumption:** does this direction introduce or rename an identifier, role, status field, or label that will be read by or applied to a person (a role name, an actor/permission value, UI copy describing someone, a status flag)? If so: is the name encoding a fact about the *system* (what it needs to route, gate, or scope) or a fact about the *person* (employment status, seniority, category)? A name that encodes a person-fact when only a system-fact was needed goes stale the moment that person's real status changes, and can read as status judgment regardless of intent — this is the git `master`→`main` class of problem, not a cosmetic one. Purely technical identifiers with no human-facing or status weight (function names, internal constants, path segments) don't need this check — only ones a person will read as describing them. If the direction introduces such a name without having asked this question, that is a HIGH objection, not BLOCKING — surface it and propose the system-fact-based alternative before the name ships.
...
Only when both are true is the loop dry. Ten rounds is the emergency brake for
genuinely unresolvable/circular objections — not the exit condition. On cap hit:
surface the unresolved tension explicitly ("Round 10 reached — objection X remains
unresolved: [state it]. User input needed before proceeding.") Do not exit silently.

**Efficiency scoring (emitted in verdict, not per-round):**
- Rounds 1–3: EFFICIENT (silent)
- Rounds 4–6: MODERATE (silent)
- Rounds 7–9: COSTLY — note in verdict: "Resolved in {N} rounds — direction may have been under-specified"
...
5. **Run-until-dry exit:** The skill keeps iterating internally until a round produces zero new objections from all angles. Only then does it surface a verdict. Emergency brake is 10 rounds — on cap hit, surface the unresolved objection explicitly and stop. It does not exit silently on cap hit, and it does not propose further revisions autonomously after surfacing the tension.
