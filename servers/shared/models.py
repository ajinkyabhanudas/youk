...
class SessionState:
    ...
    # Cross-project concept graph hits: patterns/decisions from OTHER projects that match
    # the current session's resume point. Surfaced once at session_start as context.
    # Format: [{label, type, project_slug, session_n, summary, match}]
    cross_project_concepts: list[dict] = field(default_factory=list)
...
class Proposal:
    ...
    # When True, apply_proposal requires review_required_override=True to proceed.
    # Used for net-new skills generated by Track A — human must confirm before the
    # skill is live, even when safe_types allows FILE_CREATE.
    review_required: bool = False
    # Project this proposal belongs to. Empty = legacy (treated as "youk" for filtering).
    # session_start surfaces only proposals matching the current project_slug.
    project_slug: str = ""
    ...
    def to_markdown(self) -> str:
        ...
        if self.review_required:
            lines.append("**ReviewRequired:** true")
        if self.content:
